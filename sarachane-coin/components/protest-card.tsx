...
import ProtestVoteChart from './protest-vote-chart'
...
export default function ProtestCard({ protest, onVoteUpdate }: ProtestCardProps) {
  const { id, title, description, location, category, startDate, participants, positiveVotes, negativeVotes, status, image } = protest
  ...
  // Kullanıcının bu protestoya oy verip vermediğini kontrol et
  useEffect(() => {
    if (address) {
      const voteKey = `vote_protest_${id}_${address}`;
  ...
  const handleVote = async (voteType: "positive" | "negative") => {
    ...
    const success = await sendTransaction(
      "0xVoteContract", 
      10, 
      `Protesto oyu: ${title} - ${voteType === "positive" ? "Destekliyorum" : "Desteklemiyorum"}`
    );
  ...
  const handleMultiVote = async (voteType: "positive" | "negative", amount: number) => {
    ...
    const success = await multiVote(
      ...
      `Protesto çoklu oyu: ${title} - ${voteType === "positive" ? "Destekliyorum" : "Desteklemiyorum"} (${amount} oy)`
    );
